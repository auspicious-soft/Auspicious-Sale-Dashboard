...
interface MobileTableCardProps {
  tabbingDataMobile: User[];
  isLoading?: boolean;
  error?: boolean;
}

const WebTableCard: React.FC<MobileTableCardProps> = ({ tabbingDataMobile = [], isLoading = false, error = false }) => {
  // Get all unique dates from all users' dailyEarnings
  const getAllDates = (): string[] => {
    const dates = new Set<string>();
    tabbingDataMobile.forEach((user: User) => {
  ...
  const calculateDateTotal = (date: string): number => {
    return tabbingDataMobile.reduce((total: number, user: User) => {
  ...
  if (!tabbingDataMobile.length || !dates.length) {
    return (
    ...
    <div className="table-common overflow-custom w-full sub-total-table">
      <table>
        <thead>
          <tr>
            <th>Date</th>
            {tabbingDataMobile.map((user: User) => (
