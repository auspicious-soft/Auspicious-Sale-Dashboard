...
interface SeoTableCardProps {
  tabbingDataSeo: User[];
  isLoading?: boolean;
  error?: boolean; 
}

const WebTableCard: React.FC<SeoTableCardProps> = ({ tabbingDataSeo = [], isLoading = false, error = false }) => {
  // Get all unique dates from all users' dailyEarnings
  const getAllDates = (): string[] => {
    const dates = new Set<string>();
    tabbingDataSeo.forEach((user: User) => {
  ...
  const calculateDateTotal = (date: string): number => {
    return tabbingDataSeo.reduce((total: number, user: User) => {
  ...
  if (!tabbingDataSeo.length || !dates.length) {
    return (
